...
export default function Matches(props){
    ...
    const matcheApi = useCallback(async() => {
        ...
        axios({
            ...
            if(response.status === 200)
            {
                ...
                // finished matches
                const finished = matchData.filter(matchData => matchData.status === 'FINISHED');
                setFinishedMatch(finished);

                // timed matches
                const timed = matchData.filter(matchData => matchData.status === 'TIMED' || matchData.status === "SCHEDULED");
                timed.sort((a, b) => new Date(b.utcDate) - new Date(a.utcDate)); // utc 날짜 반대정렬
                setTimedMatch(timed);
    ...
    const matchStatus = (status, utctime, index) => {
    ...
    const winnerDivision = (div, winner, score) => {
    ...
    // win & lose focus
    const matchResultFocusing = (homeKey, awayKey, winner) => {
    ...
    const matcheList = matches && matches.slice(0).reverse().map((m, index) => (
        
        <dd key={m.id}> 
            {matchResultFocusing(m.homeTeam.id, m.awayTeam.id, m.score.winner)}
            <div className='leage-title'>
                {<div><img src={m.competition.emblem} width='16' height='16' alt=''/></div>}
                {m.competition.name}
            </div>
            <div className='vs-wrap home'>
                <div>
                    <span className='ico-home'>H</span>
                    {<img src={m.homeTeam.crest} width='16' alt=''/>}
            ...
            <div className='vs-wrap away'>
                <div>
                    ...
                    {<img src={m.awayTeam.crest} width='16' height='16' alt=''/>}
